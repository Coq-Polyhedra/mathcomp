...
Definition lfun_comp_ringMixin_c := GRing.Ring.on (lfun_comp_ringType^c).
Definition lfun_ringType := Eval hnf in
  RingType 'End(vT) lfun_comp_ringMixin_c.

Definition lfun_lalgMixin := GRing.Lmodule_IsLalgebra.Build _ lfun_ringType
  (fun k x y => comp_lfunZr k y x).
Definition lfun_lalgType := Eval hnf in
  LalgType _ 'End(vT) lfun_comp_ringMixin_c lfun_lalgMixin.

Definition lfun_algMixin := GRing.Lalgebra_IsAlgebra.Build R lfun_lalgType
    (fun k x y => comp_lfunZl k y x).
Definition lfun_algType := Eval hnf in
  AlgType _ 'End(vT) lfun_comp_ringMixin_c lfun_algMixin lfun_lalgMixin.
